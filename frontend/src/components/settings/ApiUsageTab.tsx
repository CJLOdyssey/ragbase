  ...
  return (
    <div className="">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-lg p-4 text-center">
          <div className="text-xl font-bold text-[var(--color-text-primary)]">
            {usage.today_tokens.toLocaleString()}
          </div>
          <div className="text-xs text-[var(--color-text-muted)] mt-1">
            {t('api.todayTokens')}
          </div>
        ...
        <div className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-lg p-4 text-center">
          <div className="text-xl font-bold text-[var(--color-text-primary)]">
            {usage.month_tokens.toLocaleString()}
          </div>
          <div className="text-xs text-[var(--color-text-muted)] mt-1">
            {t('api.monthTokens')}
          </div>
        ...
        <div className="border-t border-[var(--color-border)] pt-5">
          ...
          <div className="flex flex-col gap-2">
            {keyUsage.map((k) => (
              <div
                key={k.label}
                className="flex items-center justify-between py-2.5 px-3.5 bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-lg"
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 ${k.is_active ? 'bg-[var(--color-success)]' : 'bg-[var(--color-text-muted)]'}`}
                  />
