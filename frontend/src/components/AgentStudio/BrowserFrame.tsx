...
export default function BrowserFrame() {
  ...
  useEffect(() => {
    const onFrame = (e: Event) => { setImg((e as CustomEvent<string>).detail); setUrl(null); };
    const onUrl = (e: Event) => { setUrl((e as CustomEvent<string>).detail); setImg(null); };
    const onClear = () => { setImg(null); setUrl(null); };
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[var(--color-border)] flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 bg-[var(--color-surface-hover)] border-b border-[var(--color-border)]">
        ...
        <button className="p-1 rounded hover:bg-[var(--color-surface-raised)]" onClick={() => {
          setImg(null);
          setUrl(null);
          import('../../stores/messageHandler').then(m => m.clearPendingBrowserUrl());
        }}>
          <X size={14} />
        </button>
      </div>
