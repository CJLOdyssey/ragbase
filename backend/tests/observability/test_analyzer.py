...
from observability.analyzer import analyze_error, analyze_trace, recent_errors_report
...
def _trace_store(events: list[dict]) -> MagicMock:
    """Store mock returning the given events for by_trace()."""
    mock_store = MagicMock()
    mock_store.by_trace.return_value = events
    return mock_store


class TestAnalyzeTrace:
    def test_empty_trace_returns_empty_events(self):
        with patch("observability.analyzer.get_store", return_value=_trace_store([])):
    ...
    def test_trace_with_no_errors(self):
        ...
        with patch("observability.analyzer.get_store", return_value=_trace_store(events)):
