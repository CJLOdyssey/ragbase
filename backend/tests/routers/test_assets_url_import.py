...
def _mock_get_user_id(user_id: str = "u1") -> None:
    def fake(request):
        return user_id

    patch("routers.assets.get_user_id", side_effect=fake).start()
...
class TestImportUrl:
    ...
    async def test_rejects_non_http_scheme(self):
        _mock_get_user_id()
        resp = MagicMock()
        resp.json.return_value = {"detail": {"error": {"code": "ATTACHMENT_003"}}}
        # Route raises error_response HTTPException; assert via call wrapper
        from fastapi import HTTPException
    ...
    async def test_imports_public_url(self):
        ...
        asset.indexed = False
        asset.source = "url"
        asset.source_ref = "https://example.com/doc.md"
        ...
        with patch(
            ...
            result = await import_asset_from_url(
                type("UrlImportIn", (), {"url": "https://example.com/doc.md", "name": None})(),
                _FakeRequest(),
            )
        mock_cls.assert_called_once()
        mock_create.assert_awaited_once()
        assert result.source == "url"
        assert result.source_ref == "https://example.com/doc.md"
